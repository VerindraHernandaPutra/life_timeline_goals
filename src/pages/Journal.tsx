  ...
  return (
    <div className="space-y-8 animate-fade-in p-4">
      <div className="flex items-center justify-between">
         <div>
            <Title order={2} className="text-stone-800 dark:text-stone-100 font-serif">Journal & Minutes</Title>
      ...
      <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
        {entries.map((entry) => (
            <div key={entry.id} className="break-inside-avoid bg-yellow-50/50 dark:bg-stone-900 p-6 rounded-2xl border border-yellow-100/50 dark:border-stone-800 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
                <div className="flex items-center gap-2 mb-3 text-stone-400 text-xs font-medium uppercase tracking-wider">
                    <Calendar size={12} />
                    {entry.date}
                </div>
                <h3 className="text-lg font-bold text-stone-800 dark:text-stone-200 mb-2 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors font-serif">{entry.title}</h3>
                <p className="text-stone-600 dark:text-stone-400 text-sm leading-relaxed mb-4 line-clamp-3">
                    {entry.content}
                </p>
                <div className="flex flex-wrap gap-2">
                    {entry.tags.map(tag => (
                        <Badge key={tag} variant="outline" color="gray" size="sm" className="bg-white/50 dark:bg-stone-800/50 border-stone-200 dark:border-stone-700 normal-case font-normal text-stone-600 dark:text-stone-400">
                            {tag}
                        </Badge>
                    ))}
                </div>
